...
(function ($, _, undefined) {
    ...
    var defaultSettings = {
        ...
        templates: {
            ...
            mentionItemHighlight: _.template('<strong title="<%= title %>"><span class="<%= cssclass %>"><%= value %></span></strong>')
        }
    };
    ...
    var utils = {
        ...
        regexpEncode: function (str) {
            //window.debug("\tersetze " + /([.*+?^=!:${}()|\[\]\/\\])/g);
            //window.debug("\tdurch " + "\\$1");
            //window.debug("\tin " + str);
    ...
    var MentionsInput = function (settings) {
        ...
        function updateValues() {
            ...
            _.each(mentionsCollection, function (mention) {
                window.debug("163 vor textSyntax")
                var textSyntax = settings.templates.mentionItemSyntax(mention) + " ";
                window.debug("164 " + mention.value);
                window.debug("165a suche in " + syntaxMessage);
                var regexp = new RegExp(utils.regexpEncode(mention.value) + "(\\s|$)", 'g');
                window.debug("165b nach regExp " + regexp);
                window.debug("165c und ersetze durch " + textSyntax);
                syntaxMessage = syntaxMessage.replace(regexp, textSyntax);
                window.debug("166 " + syntaxMessage);
            });
            ...
            _.each(mentionsCollection, function (mention) {
                ...
                switch (mention.type) {
                    case "theme":
                        formattedMention.cssclass = "highlight_theme";
                        break;
                    case "contact":
                        formattedMention.cssclass = "highlight_contact";
                        break;
                }
                formattedMention.title = "bla bla";
        ...
        function getTriggerCharsRegexp(chars) {
            var t = "(";
        ...
        function addMention(mention) {
            ...
            var regex = new RegExp(currentDataQuery, "gi");
            regex.exec(currentMessage); //Executes a search for a match in a specified string. Returns a result array, or null

            var startCaretPosition = regex.lastIndex - currentDataQuery.length; //Set the star caret position
            var currentCaretPosition = regex.lastIndex; //Set the current caret position
            window.debug(startCaretPosition);
            window.debug(currentCaretPosition);
            ...
            // TODO: check, ob nach dem mention.value ein whitespace kommt, wenn nicht, dann keins extra einfügen
            var updatedMessageText = start + mention.value + ' ' + end;
            elmInputBox.val(updatedMessageText); //Set the value to the txt area
            elmInputBox.trigger('mention');
            window.debug("245 " + mention.value);
            updateValues();
        ...
        function onInputBoxInput(e) {
            ...
            var lastIndex = -1;
            // find last index of of triggerchar
            settings.triggerChar.forEach(function (el, i) {
                var li = _.lastIndexOf(inputBuffer, el);
                if (li > lastIndex)
                    lastIndex = li;
            });

            var triggerCharIndex = lastIndex; //Returns the last match of the triggerChar in the inputBuffer
            if (triggerCharIndex > -1) { //If the triggerChar is present in the inputBuffer array
                currentDataQuery = inputBuffer.slice(triggerCharIndex).join(''); //Gets the currentDataQuery
        ...
        function resetInput(currentVal) {
            ...
            var regex = new RegExp(getTriggerCharsRegexp(settings.triggerChar) + "\\[(.*?)\\]\\((.*?):(.*?)\\)", "gi");
